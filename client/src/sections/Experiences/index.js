...
function Experiences() {
  ...
  useEffect(() => {
    const fetchExperienceStudyData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/experiences/type/?type=study')
  ...
  useEffect(() => {
      const fetchExperienceJobData = async () => {
          try {
              const response = await axios.get('http://localhost:5000/experiences/type/?ctype=job')
              console.log(response.data)
